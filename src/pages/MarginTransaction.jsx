...
import { fetchMarginTransactions } from "../api";
...
function UserList() {
  ...
  useEffect(() => {
    const fetchMarginTransactionsFunc = async () => {
      setLoading(true);
      setErr(null);
      try {
        const skip = (page - 1) * limit;
        const data = await fetchMarginTransactions({ limit, skip });
        console.log(data)
        if (data?.status) {
  ...
  return (
    <div className="container mx-auto max-w-[1320px] py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-purple-600 text-white text-center py-4 rounded-md mb-6">
        <h1 className="text-2xl md:text-4xl  font-bold">Margin Transaction</h1>
      </div>

      {/* Total Deposit */}
      <p className="text-lg font-semibold text-purple-600 mb-4">
       Total Margin Transaction: <span className="text-lg text-green-600 font-semibold">{record_count}</span>
      </p>
          ...
          <tbody>
            {transactions.map((user, index) => (
              <tr key={user._id} className="{index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} text-center font-medium border">
                ...
                <td className="px-6 py-4 border border-gray-300 ">{user.commissionAmount}</td>
                ...
                <td className="px-6 py-4 border border-gray-300">{user.transactionCharge}</td>
                <td className="px-6 py-4 border border-gray-300">{user.payableAmount}</td>
                <td className="px-6 py-4 border border-gray-300">{user.type}</td>
                <td className="px-6 py-4 border border-gray-300 text-nowrap">{user.remark } </td>
                <td className="px-6 py-4 border border-gray-300">{user.tokenId.tokenName }</td>
                <td className="px-6 py-4 border border-gray-300">{user.usdtValue}</td>
                <td className="px-6 py-4 border border-gray-300 truncate max-w-[220px] w-full"><a href="#" className="text-blue-500 underline">{user.transactionHash}</a></td>
            ...
            pages.push(
              <button
                ...
                className={`h-7 min-w-7 px-3 flex items-center justify-center rounded-md border text-sm font-medium cursor-pointer ${page === i
                    ? "bg-purple-600 text-white border-purple-600"
                    : "bg-white hover:bg-purple-100 text-gray-700 border-gray-300 hover:border-purple-500"
                  }`}
              >
                {i}
              </button>
