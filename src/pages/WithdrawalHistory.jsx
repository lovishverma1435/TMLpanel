...
import { fetchPendingWithdraws } from "../api";
...
function UserList() {
  ...
  useEffect(() => {
    const fetchPendingWithdrawsFunc = async () => {
      setLoading(true);
      setErr(null);
      try {
        const skip = (page - 1) * limit;
        const data = await fetchPendingWithdraws({ limit, skip });
        console.log(data)
        if (data?.status) {
  ...
  return (
    <div className="container mx-auto max-w-[1320px] py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-purple-600 text-white text-center py-4 rounded-md mb-6">
        <h1 className="text-2xl md:text-4xl  font-bold">withdraw Transaction</h1>
      </div>

      {/* Total Deposit */}
      <p className="text-lg font-semibold text-purple-600 mb-4">
        Total withdraw Transaction: <span className="text-lg text-green-600 font-semibold">{record_count}</span>
      </p>
          ...
          <tbody>
            {transactions.map((user, index) => (
              <tr key={user._id} className="{index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} text-center font-medium border">
                ...
                <td className="px-6 py-4 border border-gray-300 truncate max-w-[220px] w-full"><a href="#" className="text-blue-500 underline">{user.walletAddress}</a></td>
